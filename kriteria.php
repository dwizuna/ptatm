						...
						$i = 1;	
							if($result->num_rows != 0){
								while ($row = $result->fetch_assoc()) {
									...
									if($row['cost_benefit']=='cost')
										echo "<td><small class='label label-warning'>".$row['cost_benefit']."</small></td>";
									if($row['cost_benefit']=='benefit')
										echo "<td><small class='label label-success'>".$row['cost_benefit']."</small></td>";
									echo '<td><div class="btn-group">';
									echo  '<a href="edit-kriteria.php?id='.$row['id_kriteria'].'" class="fa fa-edit btn btn-info btn-sm"></a>';?>
									<?php 
									echo  '</div>
										  </td>';
									echo "</tr>";
